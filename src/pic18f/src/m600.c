...
** Last update Sat Nov 14 15:31:33 2009 texane
*/
...
m600_alarms_t m600_read_alarms(void)
{
  ...
  M600_SET_ALARM(alarms, HOPPER_CHECK);
...
m600_alarms_t m600_read_card(uint16_t* col_data)
{
...
m600_alarms_t m600_read_card(uint16_t* col_data)
{
  unsigned int i = M600_COLUMN_COUNT;
...
/* return 1 if the hopper is empty, 0 otherwise */

int m600_is_empty(void)
{
  if (M600_PIN_HOPPER_CHECK)
    return 1;
...
int m600_read_request(m600_request_t* req)
{
  /* idle until interrupt */

  osc_set_power(OSC_PMODE_PRI_IDLE);
...
void m600_write_reply(m600_reply_t* rep)
{
  serial_write((unsigned char*)rep, sizeof(m600_reply_t));
}
